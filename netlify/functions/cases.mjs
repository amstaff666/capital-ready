...
export async function handler(event) {
  ...
  if (!event.body || event.body.length > 250000) return json(413,{error:"payload_too_large"});

  let payload;
  try { payload = JSON.parse(event.body); }
  catch { return json(400,{error:"invalid_json"}); }

  if (payload.botField) return json(202,{accepted:true});
  const market = String(payload.market || "").toUpperCase();
  const envName = MARKET_ENV[market];
  if (!envName) return json(400,{error:"unsupported_market"});

  const baseUrl = process.env[envName];
  if (!baseUrl) return json(503,{error:"market_backend_not_configured",market});

  const upstream = await fetch(new URL("/v1/cases", baseUrl), {
    method:"POST",
